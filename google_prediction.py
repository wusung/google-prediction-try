...
def train(auth, model):
    ...
    url = 'https://www.googleapis.com/prediction/v1.6/training?data=%s' % \
        urllib.quote(model, '')
    url = 'https://www.googleapis.com/prediction/v1/projects/foxdie-service/trainedmodels'
    print 'URL=%s ,%s' % (url, auth)
    headers = {
...
def predict(auth, model, query):
    ...
    url = 'https://www.googleapis.com/prediction/v1.6/training/%s/predict' % \
        urllib.quote(model, '')
